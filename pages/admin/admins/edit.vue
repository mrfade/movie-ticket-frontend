...
const { data, error } = await useFetch<ApiResponse<User>>(`/admin/${route.query.id}`, {
  ...apiOptions()
})
...
const saveUser = async (): Promise<string | void> => {
  ...
  const { error } = await useFetch<ApiResponse<User>>('/admin', {
    ...apiOptions(),
    method: 'PUT',
    body: {
      id: route.query.id,
...
<template>
  <div class="p-8">
    <admin-title>Admin Edit</admin-title>
